...
def get_input_data()->pd.DataFrame:
    df1 = pd.read_csv(r"E:\Shoban-NCI\VS_Code_WS\Eng&Eval_AI\EEAI_Project\EEAI_Project\data\AppGallery.csv", skipinitialspace=True)
    df1.rename(columns={'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}, inplace=True)
    df2 = pd.read_csv(r"E:\Shoban-NCI\VS_Code_WS\Eng&Eval_AI\EEAI_Project\EEAI_Project\data\Purchasing.csv", skipinitialspace=True)
    df2.rename(columns={'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}, inplace=True)
    df = pd.concat([df1, df2])
    df[Config.INTERACTION_CONTENT] = df[Config.INTERACTION_CONTENT].values.astype('U')
    df[Config.TICKET_SUMMARY] = df[Config.TICKET_SUMMARY].values.astype('U')
    df = df.loc[(df[['y2', 'y3', 'y4']] != '').all(axis=1) & (~df[['y2', 'y3', 'y4']].isna().any(axis=1))]
    #Blank values and Nan Values are removed from y columns
...
def de_duplication(data: pd.DataFrame):
    ...
    for t in tickets.index:
        ...
        for ic in df[Config.INTERACTION_CONTENT]:
            ...
            # ic_r = sum(ic_r, [])
            #Removes None value from the list    
            ic_r = [i for i in ic_r if i is not None]

            # replace split patterns
            #******this is an overhead as ic_r is already split using split_pattern
            ic_r = [re.sub(split_pattern, "", i.strip()) for i in ic_r]

            # replace customer template
            ic_r = [re.sub(cu_pattern, "", i.strip()) for i in ic_r]

            #The empty string are not added to ic_current. Only non duplicate values are added in the list. 
            ic_current = []
            for i in ic_r:
                if len(i) > 0:
                    # print(i)
